...
import gl2.example.moviesapi.service.FilmService;
...
public class FilmController {
    ...
    public ResponseEntity<Film> patchFilm(
        ...
        if (updates.containsKey("realisateur")) {
            film.setRealisateur((Realisateur) updates.get("realisateur"));
        }
    ...
    @GetMapping("/filter/realisateur/{nomRealisateur}")
    public List<Film> filterByRealisateur(@PathVariable(required = false) String nomRealisateur) {
        System.out.println("Filtering with nom de réalisateur=" + nomRealisateur);
        return filmService.filterByRealisateur(nomRealisateur);
    }
